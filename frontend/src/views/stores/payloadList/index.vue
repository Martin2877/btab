...
import {
  getPayloadTableList,
  getPayloadDirReq,
  uploadPayloadDataReq,
  delPayloadTableList,
  getPayloadDetailTableList,
} from "@/api/table/list";
...
const openDirRequset = async () => {
  return await getPayloadDirReq();
};

const loadDataTable = async (res) => {
  return await getPayloadTableList({ ...formParams, ...params.value, ...res });
};

const loadDataDetailTable = async (res) => {
  return await getPayloadDetailTableList({ ...formParams, ...params.value, ...res });
};
...
const uploadData = async (res) => {
  return await uploadPayloadDataReq(res);
};

const delTable = async (res) => {
  return await delPayloadTableList(res);
};
...
function handleDetail(record: Recordable) {
  setTimeout(() => {
    loadDataDetailTable({search:record.sha_1}).then(
      function(resp){
