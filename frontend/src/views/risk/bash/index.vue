...
import { getPayloadTableList,getTableRiskBASHList,addTableRiskBASHList,addOnceRiskBASHList,delTableRiskBASHList } from '@/api/table/list';
import { columns } from './columns';
import { PlusOutlined } from '@vicons/antd';
import { useRouter } from "vue-router";

const bashList = ref<Array<String>>([]);

const getBASHDataTable = async () => {
  return await getPayloadTableList({pageSize:10000,page:1});
};

// 从 api 接口获取 bashList
function getBASHList () {
  getBASHDataTable().then(
    function(resp){
      var arr = new Array();
...
const addDataTable = async (res) => {
  return await addTableRiskBASHList({ ...res});
};

const addOnceDataTable = async (res) => {
  return await addOnceRiskBASHList({ ...res});
};


const loadDataTable = async (res) => {
  return await getTableRiskBASHList({ ...formParams, ...params.value, ...res });
};
...
function handleDelete(record: Recordable) {
  message.info("点击了删除");
  setTimeout(() => {
    delTableRiskBASHList(record);
    reloadTable();
  }, 500);
}

function uploadBASH(record: Recordable) {
